...
3. Building a CSR from `private_key_pem_wo`.
...
resource "stepca_certificate" "middleware_tls" {
  ...
  private_key_pem_wo           = ephemeral.tls_private_key.middleware.private_key_pem
...
- `private_key_pem_wo` (String, Sensitive, Write-Only) PEM private key used to build the CSR. Never stored in state.
